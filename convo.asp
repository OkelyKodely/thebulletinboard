...
<div style="background-color:blue;height:61px;border:0px solid black;color:#efefef;width:500px">
  <h3 style="color:yellow">Start Chatting....</h3>
</div>
...
sqlStr = "select c.id, c.categname, (select count(a.id) from posts a  where a.categid = c.id) as cnt from Categories c"
set rs = cnn.execute(sqlStr)
dim bgcolor
bgcolor = "gray"
dim ii
...
%>
<div style="background-color:<%=bgcolor%>;border:0px solid black;color:#ccc;height:50px">
  <A HREF='category.asp?cid=<%=rs("id")%>' style="color:#111;font-size:25px"><%=rs("categname")%></A> (<%=rs("cnt")%>)
</div>
<%
ii = ii + 1
rs.movenext
